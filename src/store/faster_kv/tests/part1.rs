...
fn test_auto_compaction_handle_stop_wakes_worker() {
    ...
    assert_eq!(
        handle.state(),
        crate::compaction::AutoCompactionState::Stopped
    );
}
...
#[test]
fn test_with_read_cache_invalid_config_returns_error() {
    let config = FasterKvConfig::default();
    let device = NullDisk::new();
    let invalid_cache_config = ReadCacheConfig::new(0);

    let result =
        FasterKv::<u64, u64, NullDisk>::with_read_cache(config, device, invalid_cache_config);
